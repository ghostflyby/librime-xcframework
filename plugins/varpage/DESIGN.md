...
因此 `clear_resolver` 是**可选**的：不调用也不会泄漏，只是 `user_data` 的释放时机交由插件决定（下一次注册或下一次活动时），并且仍存在一个 id 层面的固有余地——按 id 查找时，陈旧 id 可能命中一个已注册的活会话，从而清掉**那个**会话的注册。头文件里写明了这一点，也把"在会话存活期间清"标为无歧义的做法。
