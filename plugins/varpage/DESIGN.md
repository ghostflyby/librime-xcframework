...
**关于会话回收**：条目里存了 `weak_ptr<Session>` 作见证。会话 id 就是会话对象地址，因此被销毁的会话的 id 可能被下一个会话复用、任何数值比较都会说"同一个"；控制块不会，这才是让残留注册**可被识别**而不是被继承的机制。

**残留条目会被自动清掉，不需要 host 手动 `clear_resolver`。** librime 没有会话销毁通知可挂（`DestroySession`、`CleanupStaleSessions`、`CleanupAllSessions` 都只是从 map 里抹掉会话），所以"会话没了"只能靠证据发现，而证据就是那条 weak 引用。两个地方会去发现它：

1. **`Find`**：按键路径按当前 context 查表时，若该条目的会话已不在，就地丢弃。被测到的常态是分配器把死会话的 context 地址交还给下一个会话，于是下一次按键恰好查到它、顺手清掉。
2. **`OnContextChanged`**：活动会话的 composition/高亮每次变化时清扫**全部**过期条目。覆盖 `Find` 到不了的情况——残留条目落在没有活会话会去查的地址上。

因此**注册条目**的清理是自动的，`clear_resolver` 对它是可选的。它真正买到的是另外两件事：会话仍存活时停止被询问（从而在那里打开 `user_data` 的安全窗口），以及一点 id 层面的余地——按 id 查找时，陈旧 id 可能命中一个已注册的活会话，从而清掉**那个**会话的注册，所以"在会话存活期间清"是无歧义的做法。头文件里写明了这一点。

**`OnContextChanged` 里的清扫没有独立测试覆盖**，且从公开 API 无法构造：分配器总会复用 context 地址，于是 `Find` 先一步清掉了，无从区分是哪条机制。测试断言的是**契约**（残留注册在下一个会话活动后不再存在），这一点两条机制都能满足；测试注释与这里都写明了这个限度，不假装覆盖。

**`user_data` 的归属（与注册不同，需要 host 主动释放）**

注册表由插件自己清理，`user_data` **不是**。两者不能混为一谈：

| | 归属 | 谁来清理 | 何时 |
|---|---|---|---|
| 注册条目 | 插件 | 插件 | 自动：随会话消失，或活动时被扫掉 |
| `user_data` | host | **host** | **必须**，且只能在安全窗口内 |

插件只存 `void*`、只回传，**从不释放**——`void*` 没有 deleter 可调。所以宿主若分配了它，释放是**义务**而非选择，否则就是宿主那边的泄漏。

安全窗口在会话结束那一刻打开，两条路径任取其一：

- `destroy_session`（宿主自己的调用），或
- `clear_resolver`（会话仍存活时）。

从那一刻起插件不会再回传该指针（每次调用都先过存活检查），所以可以释放、也应当释放。**在此之前它必须保持有效**——插件仍可能询问；提前释放就是 use-after-free。这条由测试钉住：会话存活时 `clear_resolver` 后，resolver 不再被调用（去掉"注销"这个动作，该断言立即失败）。

**这也是不需要清理回调的原因**：回调能给的信息宿主本来就知道——`destroy_session` 是宿主自己的调用。
