...
ProcessResult VarPageSelector::ProcessKeyEvent(const KeyEvent& key_event) {
  ...
  // actions have to be intercepted before the base impl can reach its
  // page_size arithmetic; the other actions, which never read the page size,
  // fall through to the base impl below.
...
bool VarPageSelector::TurnPreviousPage(Context* ctx) {
  return varpage::PreviousPage(engine_->schema(), ctx);
}

bool VarPageSelector::TurnNextPage(Context* ctx) {
  return varpage::NextPage(engine_->schema(), ctx);
}
