...
VarPageSelector::VarPageSelector(const Ticket& ticket) : Selector(ticket) {
  ...
  // Ticket(this), so this instance's engine is the switcher and its context is
  // the panel's. Upstream identifies a switcher the same way, in the two
  // translators that drive the schema list.
  if (const auto* switcher = dynamic_cast<Switcher*>(engine_)) {
...
ProcessResult VarPageSelector::ProcessKeyEvent(const KeyEvent& key_event) {
  // The same gates the built-in selector applies before it looks at the
  // keymaps. They are repeated here rather than inherited because the page
  // actions have to be intercepted before the base impl can reach its page_size
  // arithmetic; the other actions, which never read the page size, fall through
  // to the base impl below.
  if (key_event.release() || key_event.alt() || key_event.super())
    return kNoop;
  Context* ctx = engine_->context();
  if (ctx->composition().empty())
    return kNoop;
  if (const Segment& current_segment(ctx->composition().back());
      !current_segment.menu || current_segment.HasTag("raw"))
    return kNoop;

  const TextOrientation text_orientation =
      IsVerticalText(ctx) ? Vertical : Horizontal;
  const CandidateListLayout candidate_list_layout =
  ...
  const auto binding = keymap.find(key_event);
  ...
  // The select keys. Their slot arithmetic is page-relative, so this is the
  // third and last page-dependent entry point and has to be computed here; the
  // base impl would resolve the slot against a fixed page.
  const int ch = key_event.keycode();
  int slot = -1;
  // Read at key time, not cached at construction: a script is allowed to swap
  // the select keys for the duration of one keystroke.
  if (const string& select_keys(engine_->schema()->select_keys());
      !select_keys.empty() && !key_event.ctrl() && ch >= 0x20 && ch < 0x7f) {
    if (const size_t pos = select_keys.find(static_cast<char>(ch));
        pos != string::npos)
...
bool VarPageSelector::TurnNextPage(Context* ctx) const {
  return varpage::NextPage(engine_->schema(), ctx, !switcher_instance_);
}

bool VarPageSelector::SelectSlot(Context* ctx, const int slot) const {
