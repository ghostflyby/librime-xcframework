...
- Give each local plugin its own `.clang-format`, so the plugin directory describes itself and could be split out without losing its style. Keep them identical: `BasedOnStyle: Chromium` with `SortIncludes: false`, which is upstream librime's own configuration - the plugin is compiled into librime with its source inside librime's tree, and `SortIncludes` stays off because the include order in these files is deliberate. Format with `xcrun clang-format`, so formatting follows the toolchain the build uses. This applies to the local plugins only; the submodule plugins keep upstream's formatting untouched.
...
- Run upstream librime's suite, and keep it running against the patched, plugin-merged source this repository builds from (`BUILD_TESTS=1 scripts/build-one-arch.sh macos-arm64`). The point is to catch the packaging layer breaking librime itself; a suite run against an unpatched checkout would report nothing about this repository. `package` depends on that job, so a failing suite stops a release.
- Keep the test procedure in one place: `.github/workflows/run-tests.yml` is a reusable workflow, called by `build.yml` (whose packaging job depends on it) and by `test.yml`, the pull request check. Do not inline it back into `build.yml`, and do not give `build.yml` a `pull_request` trigger - its jobs are the release path, and a second event would mean guarding every slice and the packaging job with an event condition that every later edit has to remember.
- The pull request check only covers changes that can affect what is built or merged (the paths listed in `test.yml`), and it is a report rather than a gate: `main` has no required status checks, so a failure is visible but does not block a merge.
- Verify the committed headers without a build: `scripts/verify-committed-headers.sh` runs in the pull request check as its own job. It exists because a local plugin's public header is committed twice - beside the plugin, which is what `install_plugin_headers` exports, and under `Sources/RimeHeaders/include`, which is what consumers import - and nothing else checks that the two agree. It also compiles the umbrella as a module and runs the API-notes and Swift-name checks the release runs, so a broken header fails on the pull request rather than while preparing a release.
...
- Behavioral tests live with the plugin they cover (`plugins/<name>/tests/`, self-locating so they move with it) and drive real input sessions against a shared librime, because the build-time gates cannot cover behavior. `verify_merged_plugins` checks merged module symbols in the static archive (and only there); nothing checks a module's runtime behavior, so a page key landing in the wrong place - or a lost `"paging"` tag, which is silent - would ship unnoticed. Upstream's suite cannot cover this either: it has no `selector` tests at all.
- Register a plugin's behavioral test with `add_test`, guarded by `BUILD_TEST AND BUILD_SHARED_LIBS`, so one `ctest` run covers it and upstream's suite together. Register it as a test *command*, not a CMake target: a plugin's CMakeLists is processed from `add_subdirectory(plugins)`, which upstream does before `add_subdirectory(src)` creates the rime target, so an executable defined there could not link it. That is also why it compiles itself at test time instead of being a gtest binary. `run_tests` verifies the registration is present before trusting a ctest run - a conditional registration that silently did not happen would leave a clean run of upstream's suite alone as the whole report.
